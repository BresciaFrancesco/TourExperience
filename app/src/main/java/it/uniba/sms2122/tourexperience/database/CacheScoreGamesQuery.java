...
public class CacheScoreGamesQuery {
     ...
     * @return true o false.
     */
    public boolean saveOne(final SQLiteDatabase db, final GameTypes gameType, final int score) {
        ContentValues cv = new ContentValues();
        cv.put(COLUMN_GAME_TYPE, gameType.toString());
        cv.put(COLUMN_SCORE, score);
        if (db.insert(CACHE_SCORE_TABLE, null, cv) == -1) {
            cv = new ContentValues();
            int currentScore = getScore(db, gameType);
            cv.put(COLUMN_SCORE, currentScore + score);
            return db.update(CACHE_SCORE_TABLE, cv, COLUMN_GAME_TYPE+"=?", new String[]{gameType.toString()}) > 0;
        }
        return true;
    }
     ...
     */
    public int getScore(final SQLiteDatabase db, final GameTypes gameType) {
        final String query = "SELECT " + COLUMN_SCORE + " FROM " + CACHE_SCORE_TABLE +
            " WHERE " + COLUMN_GAME_TYPE + "='"+gameType.toString()+"'";
